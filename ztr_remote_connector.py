...
def aruba_tsa_timestamp(doc_hash: str) -> dict:
    """
    Request an eIDAS-qualified timestamp from Aruba PEC free TSA.
    
    Endpoint: https://freetsa.aruba.it/tsa/
    Protocol: RFC 3161 TimeStampReq/TimeStampResp
    
    Returns:
        {"token": "<base64 TSR>", "status": "granted"}
    
    Raises:
        Exception on any failure (network, TSA rejection, etc.)
    """
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            return {"token": token_b64, "status": "granted"}
        else:
            raise Exception(f"TSA HTTP {resp.status_code}")

    finally:
        for path in [hash_file, req_file]:
